...
const SectorCard: React.FC<SectorCardProps> = ({ title, subtitle, icon, color, notifications, onClick }) => {
  const { isRTL } = useLanguage();
  
  return (
    <div 
      onClick={onClick}
      className={`card-enhanced min-w-[200px] h-[180px] p-6 cursor-pointer group relative overflow-hidden ${color}`}
    >
